...
#%%
#############################################################################
# This program reads a LAS csv file that has been prepared for fitting a 
# neural network to predict bathymetry. It uses SciKit-learn's MLP classifier
# and outputs various summary statistics.  Validation is done internally
# during the fitting of the neural network. The main goal of this at this
# point is to output a csv containing only the pulses that were classified
# as Bathymetry using the optimised threshold. This will then be
# subsequently read into this program to see what the goodness-of-fit
# is for the reduced data set.
#################### ACCURACY#####################################
...
# attributes is returned.Start with 1m grid then do 5m grid.
...
def plot_roc(actual, predicted, filename,ax):
    ...
    ax.set_title('NN ROC (Bathymetry)'+filename)
...
#file_list=['Test_dfbath_NN_df2016_430000e_2707500n.csv']
#file_list=['Test9_highp_dfbath_NN_df2016_430000e_2707500n.csv']
#file_list= ['df2016_430000e_2707500n_w_SBET_edge_scan_drctn_xy.csv',
...
############################################################################
allnameprefix='m_FinalAnalysis1_MLP_IndividTiles_ReduMainEff_1_5m_elev'
...
######## CHANGE HERE AND BELOW IF NOT NORMALISING VARIABLES ###############
    tilenameprefix='m_FinalAnalysis1_MLP_IndividTiles_ReduMainEff_1_5m_elev_'
# dfbathnameprefix is only used if I am outputting a slice of the confusion
# matrix for subsequent analysis.
#    dfbathnameprefix='FinalAnalysis1_MLP_AllMainEffects_1m_5m_grids_'
...
#    dfin['abs_scnangl']=dfin['scan_angle'].abs()
######################################################################
...
# not an issue with MLP so all three variables are retained here.
# (13 vars yielding a structure of (20,20,20))
...
# including elevation). (17 vars) MLP structure = 4 hidden layers and 25
# neurons/layer: (25,25,25,25)
#    Xcols=['num_returns','return_no','first_of_many','last','last_of_many',
#           'stdXYZ','stdYwPtRl','maxabsdev','abs_devia',
#           'scan_direct','azim2plse','inciangle','pls_frm_hdng',
...
# The following is for 5m grids -- topo main effects and topo orthog main
# effects. (16 vars) MLP structure = 4 hidden layers and 25
# neurons/layer: (25,25,25,25)
#    Xcols=['num_returns','return_no','first_of_many','last','last_of_many',
#           'stdXYZ','stdYwPtRl','maxabsdev','abs_devia',
#           'scan_direct','azim2plse','inciangle','pls_frm_hdng',
...
# The following is for 5m grids -- topo main effects and topo orthog main
# effects. (17 vars) MLP structure = 4 hidden layers and 25
# neurons/layer: (25,25,25,25)
#    Xcols=['num_returns','return_no','first_of_many','last','last_of_many',
#           'stdXYZ','stdYwPtRl','maxabsdev','abs_devia',
#           'scan_direct','azim2plse','inciangle','pls_frm_hdng',
...
# The following is for 5m grids -- topo main effects and topo orthog main
# effects. (19 vars) MLP structure = 4 hidden layers and 25
# neurons/layer: (28,28,28,28,28)
#    Xcols=['num_returns','return_no','first_of_many','last','last_of_many',
#           'stdXYZ','stdYwPtRl','maxabsdev','abs_devia',
#           'scan_direct','azim2plse','inciangle','pls_frm_hdng',
...
# The following is for 5m grids -- topo main effects and topo orthog main
# effects. (21 vars) (30,30,30,30,30)
#    Xcols=['num_returns','return_no','first_of_many','last','last_of_many',
#           'stdXYZ','stdYwPtRl','maxabsdev','abs_devia',
#           'scan_direct','azim2plse','inciangle','pls_frm_hdng',
...
# Get rid of variables that have missing values (usually because they
# have no variablility -- i.e., Vmin=Vmax.)
...
# Fit model. alpha is l2 regularisaiton parameter. Value chosen based on
# logisTIc regression experience.
    lmodel=MLPClassifier(solver='lbfgs', alpha=modelpenalty,
...
# Fit null logistic model.
    ...
    nullmodel=MLPClassifier(solver='lbfgs', alpha=modelpenalty,
...
# confusion matrix -- i.e., decision threshold =0.50.
    ...
    print('\nOPTIMUM THRESHOLD (all obs w/ p>Optimum is Bathymetry):',
        round(opt_thresh.loc[0,'thresholds'],4),'\n\t(FPR:', round(opt_thresh.loc[0,'fpr'],3),
